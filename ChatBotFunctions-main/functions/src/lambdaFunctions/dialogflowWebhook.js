...
exports.dialogflowWebhook = functions.https.onRequest(async (request, response) => {
  ...
  function makeAppointment(agent) {
    ...
    return createCalendarEvent(dateTimeStart, dateTimeEnd, appointmentType, emailAttendee, description).then(() => {
      ...
      agent.add(`Ok, let me see if we can fit you in. ${appointmentTimeString} (PST) is fine!. I've sent you an email.`);
    }).catch((e) => {
      console.log("\n\nERROR = " + e);
  ...
  function sendResume(agent) {
    try {
      ...
      agent.add("Check your mailbox, please. I've sent you email.");
    } catch (e) {
      console.log("\n\nERROR = " + e);
      // need to store data to send it later.
      agent.add("Ok. I'll send you email shortly.");
    }
  }
